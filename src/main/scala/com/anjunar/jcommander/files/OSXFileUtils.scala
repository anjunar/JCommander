...
class OSXFileUtils extends AbstractFileUtils {
  ...
  def ctxItem(text: String, iconName: String)(action: => Unit): MenuItem = {
    ...
    val lbl = new Label(text) { style = "-fx-font-size: 13px;" }
    val box = new HBox(10, icon, lbl) { alignment = Pos.CenterLeft; style = "-fx-padding: 6 4 6 4;" }
  ...
  override def fileContext(files: Seq[String], event: MouseEvent): Unit = {
    if (files.isEmpty || contextMenuOpen) return
    ...
    val openWithApps = Seq("open", "subl", "code", "nano", "vim")
    ...
    val executeItem = ctxItem("Execute", "mdi2p-play") { if single then executeFile(fHead) }
    val copyItem = ctxItem("Copy", "mdi2c-content-copy") { FileClipboard.copyMany(files) }
    val pasteItem = ctxItem("Paste", "mdi2c-content-paste") { FileClipboard.pasteToDirectory(parentDir) }
    ...
    val duplicateItem = ctxItem("Duplicate", "mdi2c-content-copy") {
      new Thread(() => duplicateFiles(files)).start()
    }

    val deleteItem = ctxItem("Delete", "mdi2d-delete") {
      val command = inject(classOf[DeleteCommand])
      command.execute()
    }

    val extractItem = ctxItem("Extract here", "mdi2f-folder-open") {
      new Thread(() => extractFiles(files)).start()
    }

    val symlinkItem = ctxItem("Create symlink", "mdi2l-link") {
      new Thread(() => createSymlink(files)).start()
    }

    val terminalItem = ctxItem("Open Terminal Here", "mdi2c-console") {
      new Thread(() => console(Paths.get(files.head).getParent.toAbsolutePath.toString)).start()
    }

    val propertiesItem = ctxItem("Properties", "mdi2c-cog") {
  ...
  def processFiles(
    ...
    val moveToRecycleBinBox = new CheckBox("Move to Recycle Bin") { selected = true }
    ...
    val confirmDialog = new ThemedDialog[ButtonType] {
      ...
      dialogPane.content = new VBox(10) {
        if (!isDelete) children += replaceExistingBox else children += moveToRecycleBinBox
      }
    }
  ...
  def showPropertiesDialog(files: Seq[String]): Unit = {
    ...
    val octal = new TextField { text = "" }
    val ownerField = new TextField { promptText = "owner" }
    val groupField = new TextField { promptText = "group" }
    ...
    def setState(cb: CheckBox, values: Seq[Boolean]): Unit = {
      ...
      if (values.forall(_ == true)) { cb.indeterminate = false; cb.selected = true }
      else if (values.forall(_ == false)) { cb.indeterminate = false; cb.selected = false }
      else { cb.indeterminate = true }
    }

    def applyMixedPermissions(perms: Seq[String]): Unit = {
      def col(i: Int): Seq[Boolean] = perms.map(_.charAt(i) != '-')
      setState(ownerR, col(0)); setState(ownerW, col(1)); setState(ownerX, col(2))
      setState(groupR, col(3)); setState(groupW, col(4)); setState(groupX, col(5))
      setState(otherR, col(6)); setState(otherW, col(7)); setState(otherX, col(8))
    }

    def applyPermissionsToChecks(perm: String): Unit = {
      Seq(ownerR, ownerW, ownerX, groupR, groupW, groupX, otherR, otherW, otherX).foreach { cb =>
        cb.allowIndeterminate = true; cb.indeterminate = false
      }
    ...
    if (files.size == 1) {
      ...
      val owners = files.map { f =>
        val p = Paths.get(f); Files.readAttributes(p, classOf[PosixFileAttributes]).owner().getName
      }

      val groups = files.map { f =>
        val p = Paths.get(f); Files.readAttributes(p, classOf[PosixFileAttributes]).group().getName
      }

      if (owners.distinct.size == 1) ownerField.text = owners.head else ownerField.text = ""
      if (groups.distinct.size == 1) groupField.text = groups.head else groupField.text = ""
      octal.text = ""
    }

    def installMixedFix(cb: CheckBox): Unit = {
      cb.onAction = _ => {
        if (cb.indeterminate()) { cb.indeterminate = false; cb.selected = true }
        updateFromChecks()
      }
    }

    Seq(ownerR, ownerW, ownerX, groupR, groupW, groupX, otherR, otherW, otherX).foreach(installMixedFix)

    def updateFromChecks(): Unit = {
      if (Seq(ownerR, ownerW, ownerX, groupR, groupW, groupX, otherR, otherW, otherX).exists(_.indeterminate())) {
        octal.text = ""; return
      }

      def bits(a: (Boolean, Boolean, Boolean)) = a match {
      ...
      val o = bits((ownerR.selected(), ownerW.selected(), ownerX.selected()))
      val g = bits((groupR.selected(), groupW.selected(), groupX.selected()))
      val ot = bits((otherR.selected(), otherW.selected(), otherX.selected()))
    ...
    octal.text.onChange { (_, _, _) =>
      if (octal.text.value.matches("[0-7]{3}")) {
        ...
        ownerR.indeterminate = false; ownerW.indeterminate = false; ownerX.indeterminate = false
        groupR.indeterminate = false; groupW.indeterminate = false; groupX.indeterminate = false
        otherR.indeterminate = false; otherW.indeterminate = false; otherX.indeterminate = false
    ...
    def showError(msg: String): Unit = {
      val d = new ThemedDialog[Unit] {
        ...
        dialogPane.buttonTypes = Seq(ButtonType.OK)
        dialogPane.content = new VBox { children = Seq(new Label(msg)) }
      }
      d.showAndWaitDialog()
    }

    def validateOwner(n: String): Boolean =
      if (n.trim.isEmpty) true else Seq("id", "-u", n.trim).! == 0

    def validateGroup(n: String): Boolean =
      if (n.trim.isEmpty) true else Seq("dscl", ".", "-read", s"/Groups/${n.trim}").! == 0
    ...
    permGrid.hgap = 10; permGrid.vgap = 6
    permGrid.add(new Label("Owner"), 0, 0); permGrid.add(ownerBox, 1, 0)
    permGrid.add(new Label("Group"), 0, 1); permGrid.add(groupBox, 1, 1)
    permGrid.add(new Label("Other"), 0, 2); permGrid.add(otherBox, 1, 2)

    val ogGrid = new GridPane
    ogGrid.hgap = 10; ogGrid.vgap = 6
    ogGrid.add(new Label("Owner"), 0, 0); ogGrid.add(ownerField, 1, 0)
    ogGrid.add(new Label("Group"), 0, 1); ogGrid.add(groupField, 1, 1)
    ...
    apply.onAction = _ => {
      ...
      if (!validateOwner(ownerName)) { showError("Owner does not exist."); return }
      if (!validateGroup(groupName)) { showError("Group does not exist."); return }
      ...
      new Thread(() => {
        files.foreach { f =>
          ...
          if (recursive.selected())
            Seq("osascript", "-e", s"""do shell script "chmod -R $newOct '$f'" with administrator privileges""").!
          else
            Seq("osascript", "-e", s"""do shell script "chmod $newOct '$f'" with administrator privileges""").!
          ...
          if (ownerName.nonEmpty || groupName.nonEmpty) {
            ...
            if (recursive.selected())
              Seq("osascript", "-e", s"""do shell script "chown -R $spec '$f'" with administrator privileges""").!
            else
              Seq("osascript", "-e", s"""do shell script "chown $spec '$f'" with administrator privileges""").!
          }
        }
        ()
      }).start()
